...
public class SlideBaseActivity2222 extends BaseActivity {
    ...
    class SwipeLayout extends FrameLayout {
            ...
            if (canSwipe) {
                ...
                switch (action) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        // TODO 为了滑动返回页面的时候，由于上一个activity 的view缩放了，导致有白边，效果不理想，换成黑色背景
                        // TODO 将上一个activity 的 DecorView 根布局背景设置成黑色(返回之后记得设置成透明)
                        ...
                        lastX = currentX;
                        updateBackgroundColor(event.getRawX());
                        break;
        ...
        private void updateBackgroundColor(float x) {
            // TODO 根据偏移量
            if (evaluator == null) {
                evaluator = new ArgbEvaluator();
            }
            int startColor = Color.parseColor("#99000000");
            int endColor = Color.parseColor("#00000000");
            int evaluate = (int) evaluator.evaluate(x / screenWidth, startColor, endColor);
